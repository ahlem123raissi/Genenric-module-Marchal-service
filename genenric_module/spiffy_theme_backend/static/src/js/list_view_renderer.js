    ...
    async onColumnFilter(ev) {
        ...
        if (['datetime', 'date'].includes(fieldType)) {
            ...
            tempInput.addEventListener('change', () => {
                if (tempInput.value) {
                    const dateObj = new Date(tempInput.value);
                    const pad = (num) => num.toString().padStart(2, '0');

                    const displayDate = `${pad(dateObj.getMonth() + 1)}/${pad(dateObj.getDate())}/${dateObj.getFullYear()} ` +
                                        `${pad(dateObj.getHours())}:${pad(dateObj.getMinutes())}:${pad(dateObj.getSeconds())}`;

                    const isoDate = `${dateObj.getFullYear()}-${pad(dateObj.getMonth() + 1)}-${pad(dateObj.getDate())} ` +
                                    `${pad(dateObj.getHours())}:${pad(dateObj.getMinutes())}:${pad(dateObj.getSeconds())}`;

                    originalInput.value = displayDate;

                    const domain = [[columnName, '>=', isoDate]];
                    const domainString = JSON.stringify(domain);
                    console.log("Domain filter applied:", domainString);
            ...
            tempInput.addEventListener('blur', cleanUp);
        }
    },

...
class ProductProduct(models.Model):
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            if not rec.no_auto_reorder:
                rec.reorder_rules_update()
        return records

    # -- Write
    def write(self, vals):
        res = super().write(vals)
        records = self.with_context(del_existing=True).filtered(
            lambda rec: not rec.no_auto_reorder
        )
        if "categ_id" in vals:
            records.with_context(attrs_only=False).reorder_rules_update()
        if "attribute_value_ids" in vals:
            records.with_context(attrs_only=True).reorder_rules_update()
        return res
    ...
    def reorder_rules_update(self):
        ...
        for rec in self:
            ...
            # Skip only service products; allow stockable ('product')
            # and consumable ('consu').
            if rec.type == "service" or rec.no_auto_reorder:
                continue
